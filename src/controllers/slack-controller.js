...
console.log(`Slack Bot watching for commands: ${_.keysIn(commands)}`);
...
app.get('/', (req, res) => {
  res.send('<h2>MillennialSpeak</h2><p>This app pairs best with slack or discord</p>');
});
...
const server = app.listen(process.env.PORT, () => console.log(`Speak(Slack) listening on port ${process.env.PORT}`));
